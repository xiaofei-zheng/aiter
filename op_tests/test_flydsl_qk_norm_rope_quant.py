...
def test_flydsl_qk_norm_rope_quant(
    ...
    *,
    ...
    _ikw = dict(dtype=torch.bfloat16, device=device, mode=init_mode, scale=init_scale, val=init_val)
...
@benchmark()
def test_flydsl_swa_write(T, H, D, RD, mode, *, seed=0, init_mode="normal", init_scale=0.1, init_val=1.0):
    torch.manual_seed(seed)
    device = torch.device("cuda")
